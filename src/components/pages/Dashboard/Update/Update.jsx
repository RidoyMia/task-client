...
const Update = () => {
    ...
    const onSubmit = (data) => {
        setLoading(true)
        console.log(data);
        axios.patch(`https://scic-task-server.vercel.app/api/v1/task/totalTask/${id}`,data).then(res =>{
      ...
      <select className='w-full px-5  py-4 mt-4 bg-gray-300 rounded-2xl' {...register("process")}>
        ...
        <option value="completed">completed</option>
                    ...
                    <div className='flex justify-center items-center align-middle mt-10'>
                        <button className='py-2 px-10 bg-green-800 text-white rounded-lg'>Update now</button>
                    </div>
                </form>
            </div>
        </div>
